...
### **Решение:** 
Для отображения такого большого количества зомби, вместо отрисовки текстуры для каждого зомби, мы можем поместить в каждый класс ключ-ссылку (перечисление TYPES), указывающий на соответствующую текстуру в типе данных (map<TYPES, CharacterTexture\*>), которая представляет собой flyweights factory.
